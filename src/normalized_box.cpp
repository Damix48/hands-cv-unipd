...
NormalizedBox::NormalizedBox(float cx_, float cy_, float w_, float h_) {
  if (cx_ > 1 || cx_ < 0) {
    throw std::invalid_argument("Normalized center x must be in range [0, 1]");
  }
  if (cy_ > 1 || cy_ < 0) {
    throw std::invalid_argument("Normalized center y must be in range [0, 1]");
  }
  if (w_ > 1 || w_ < 0) {
    throw std::invalid_argument("Normalized width must be in range [0, 1]");
  }
  if (h_ > 1 || h_ < 0) {
    throw std::invalid_argument("Normalized height must be in range [0, 1]");
  }
